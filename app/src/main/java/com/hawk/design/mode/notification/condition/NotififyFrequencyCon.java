...
public  class NotififyFrequencyCon extends AConditionAction {
    ...
    public boolean checkCondition() {
        NLog.d(TAG, "NotififyFrequencyCon checkCondition");
    ...
    public  boolean haveAlreadyNotifiedIn48Hours(long lastNotify, long serverInterval){
        //for test
        NLog.d(TAG, "NotififyFrequencyCon haveAlreadyNotifiedIn48Hours interval = %d 小时, server interval = %d 小时", ((System.currentTimeMillis() - lastNotify) / NotificationConstants.ONE_HOUR_MILLES), serverInterval / NotificationConstants.ONE_HOUR_MILLES);
        // 距离上一次间隔在48小时内
        if (System.currentTimeMillis() - lastNotify < serverInterval ) {
            NLog.e(TAG, "NotififyFrequencyCon 距离上一次通知此类通知间隔不足 %d 小时, do not notify",  serverInterval / NotificationConstants.ONE_HOUR_MILLES);
            return true;
        }
        NLog.d(TAG, "NotififyFrequencyCon距离上一次通知此类通知间隔超过 %d 小时, do  notify",  serverInterval / NotificationConstants.ONE_HOUR_MILLES);
        return false;
    }
